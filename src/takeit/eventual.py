...
class EventualQueue:
    """
    Eventually-fires callables, deferring them by one reactor turn.

    THREAD SAFETY: All methods on this class MUST be called from the
    reactor thread. There are no locks; concurrent calls from worker
    threads will race on `_calls` and on the timer-creation check.
    To schedule from another thread, hop back first:
    ``reactor.callFromThread(eq.eventually, fn, *args)``.

    Used as the single source of asynchrony for the wormhole, so this
    contract is foundational — every Boss / Mailbox / Observer caller
    today honors it.
    """
