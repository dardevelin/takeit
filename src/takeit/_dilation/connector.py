...
from ..util import to_unicode, provides
...
class Connector:
    ...
    # Kept for compatibility with callers that still pass this argument; takeit
    # has dropped the transit relay entirely, so this value is ignored.
    ...
    def __attrs_post_init__(self):
        ...
        # addresses are gathered alongside LAN listener addresses in
        # _start_listener.
    ...
    def _gather_stun_hints(self, portnum):
        """Send STUN Binding Requests in parallel; publish reflexive hints
        as they arrive. STUN gives us our NAT's public UDP-mapping IP — we
        publish it as a TCP DirectTCPV1Hint with the listener's TCP port.
        ...
        def _publish_one(result):
            ip, _udp_port = result
            if ip in seen:
                return
            seen.add(ip)
            # Lower priority (-1.0) than LAN hints — peers will try LAN
            # first and only fall back to public-IP hints if no LAN hint
            # works.
            hint = DirectTCPV1Hint(to_unicode(ip), portnum, -1.0)
            self.listener_ready([hint])
    ...
    def _use_hints(self, hints):
        ...
        hint_status = []
        for h in hints:
            # ignore any non-direct hints (e.g. legacy RelayV1Hint from a peer
            # that hasn't dropped the transit relay yet)
        ...
        # STUN-derived reflexive candidates from the peer arrive as
        # DirectTCPV1Hint with priority -1.0 (lower than LAN). We try them
        # all immediately; a real happy-eyeballs ordering is a future
        # refinement (see `consider()` for the current selection logic).
    ...
    def _connect(self, ep, description):
        f = OutboundConnectionFactory(self, description)
        d = ep.connect(f)
