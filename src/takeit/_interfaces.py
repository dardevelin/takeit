...
class IInputHelper(Interface):
    """
    Drives interactive entry of a takeit code (e.g. for `takeit receive`).

    The helper is thread-affine: all methods must be called from the
    construction thread. Use it from the readline thread via
    ``twisted.internet.threads.blockingCallFromThread``.
    """

    def get_word_completions(prefix):
        """
        Return a set of word completions for the given prefix.

        The prefix is the entire code typed so far, including any hyphens.
        Completions are returned as full strings (including the prefix), so
        readline can show them directly. Results for non-final words include
        a terminating hyphen; the final word does not.

        Completion alternates between the PGP wordlist's odd and even halves
        based on hyphen-count: the first word from the odd list, the second
        from the even list, and so on.

        Synchronous and local — there is no server round-trip.

        :param str prefix: the code as typed so far
        :return: a set of potential completions
        :rtype: set[str]
        """

    def choose_words(code):
        """
        Submit the final code.

        Terminates the code-entry process. It does not return anything, but
        causes ``w.get_code()`` to fire and initiates the wormhole
        connection. Pass the entire code (e.g. ``"purple-sausages-mocha"``).

        May only be called once; a second call raises ``AlreadyChoseWordsError``.

        :param str code: the full takeit code
        :return: None
        """
